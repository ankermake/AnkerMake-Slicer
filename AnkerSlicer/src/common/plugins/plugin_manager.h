...
#include "interfaces/anker_plugin.h"
#include "interfaces/anker_edit_plugin.h"
...
public slots:
    void recMsgfromPlugin(PluginMessageData metadata) {
        
        if(AkConst::Plugin::FDM_SEND2ALL == metadata.dest){
            emit sendMsg2MessageProcessing(metadata);
            for(auto * pInterface : allPlugins){
                pInterface->recMsgfromManager(metadata);
            }
            return;
        }

        
        if(AkConst::Plugin::AK_MAIN_WINDOW == metadata.dest){
            emit sendMsg2MessageProcessing(metadata);
            return;
        }

        //qDebug() << this << this->thread() << __LINE__ << __FUNCTION__ ;
        for(auto * pInterface : allPlugins){
            if(pInterface->pluginName() == metadata.dest){
                //qDebug() << this << this->thread() << __LINE__ << __FUNCTION__ << metadata.dest;
                pInterface->recMsgfromManager(metadata);
                break;
            }
        }
...
signals:
    void sendMsg2MessageProcessing(PluginMessageData metadata);
...
public:
    ...
    QVector<AnkerPlugin     *> getAnkerPlugins()    {return ankerPlugins;}
    QVector<AnkerEditPlugin *> getAnkerEditPlugins(){return ankerEditPlugins;}
private:
	//all plugins
    ...
    //  add @2022-03-03 by CL
    
    QVector<AnkerPlugin     *> ankerPlugins;
    QVector<AnkerEditPlugin *> ankerEditPlugins;
