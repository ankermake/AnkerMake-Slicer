...
public:
	...
	inline bool operator== (const CMeshO& oth) { return true; }

	friend void swap(CMeshO& m1, CMeshO& m2);

	Box3m trBB() const;

	bool pick(CHLineSegment3DShowObjPtr lineSegment, CHPointOnMesh& result);


	
	Box3m calCurrentAABB();

	
	
	void adjustToSuitableSize(Box3m workPlat, double ratio = 0.5);

	
	void adjustRotCenterXYToPrintCenter(double moveToX = 0, double moveToY = 0);
