      ...
      m_messageDialog(nullptr)
{
    ...
    if(m_service) {
        quickView->rootContext()->setContextProperty("machineParameter", m_service);
    }
    ...
    if(node) {
     ...
     qDebug() << " value ====" << list2.find("machine_extruder_count")->toList();
        if(!list2.isEmpty()) {
            quickView->rootContext()->setContextProperty("machineList", QVariant::fromValue(list2));
        }
    }
    quickView->setSource(QUrl::fromLocalFile(":/qml/PreferenceSettings/MachineView.qml"));

    connect((QObject *)(quickView->rootObject()),SIGNAL(qmlCurrentMachineChanged(QString)),m_service,SLOT(currentMachineChanged(QString)), Qt::QueuedConnection);
    connect((QObject *)(quickView->rootObject()),SIGNAL(qmlDeleteCustomMachine(QString)),m_service,SLOT(deleteCustomMachine(QString)), Qt::QueuedConnection);
    connect((QObject *)(quickView->rootObject()),SIGNAL(qmlCreateMachine(QString)),m_service,SLOT(createMachine(QString)), Qt::QueuedConnection);
    connect((QObject *)(quickView->rootObject()),SIGNAL(qmlCurrentTabChanged(int)),m_service,SLOT(onTabSwitched(int)));

    //rename
    connect((QObject *)(quickView->rootObject()),SIGNAL(qmlRenameCustomMachine(QString)),this,SLOT(renameCustomMachine(QString)),Qt::QueuedConnection);
