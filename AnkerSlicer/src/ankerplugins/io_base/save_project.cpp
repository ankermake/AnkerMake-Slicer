...
QDomElement matrix44mToXML(const Matrix44m &m, bool binary, QDomDocument &doc)
{
    ...
    QDomText nd;qDebug()<<"binary11:"<<binary;
	if (binary) {
...
QDomElement meshModelToXML_2(const MeshModel& mp, QDomDocument &doc, const QString& path, bool binary, bool saveViewState, const MLRenderingData& rendOpt = MLRenderingData())
{
    QDomElement meshElem = doc.createElement("MLMesh");
    meshElem.setAttribute("label", mp.label());

    QString path1;
    path1.truncate(path.lastIndexOf("/"));
    QString fileNameProject="./"+mp.label();

    QString sourcefile = mp.fullName();
    FileCopy(sourcefile, fileNameProject,1);

    meshElem.setAttribute("filename", fileNameProject);
    meshElem.setAttribute("visible", saveViewState? mp.isVisible() : true);
    meshElem.setAttribute("idInFile", mp.idInFile());
...
QDomDocument meshDocumentToXML_2(
        const MeshDocument &md,
        ...
        const std::vector<MLRenderingData>& rendOpt)
{
    ...
    for(const MeshModel& mmp : md.meshIterator()) {
        if ((!onlyVisibleLayers) || (mmp.isVisible())) {
            ...
            if (rendOpt.size() == md.meshNumber())
            {
                meshElem = meshModelToXML_2(mmp, ddoc, path, binary, true, rendOpt[i]);
            }
            else
                meshElem = meshModelToXML_2(mmp, ddoc, path, binary, false);
            mgroot.appendChild(meshElem);
        }
        ++i;
    }
        ...
        vcg::CallBackPos* cb)
{
    ...
    int findIndex2 = filename.lastIndexOf("/");
    QString runPth = filename.mid(0,findIndex1);
    QString runPth1 = filename.mid(findIndex2,findIndex1-findIndex2);
    QString runPth2 = filename.mid(findIndex2);
    ...
    QString path = runPth + runPth1 + ".anker";

    QFileInfo fi(path);
    bool binary = fi.suffix().toUpper() == "MLB";
    QDir tmpDir = QDir::current();
    QDir::setCurrent(fi.absoluteDir().absolutePath());
    QDomDocument doc = mlp::meshDocumentToXML_2(md, path, onlyVisibleLayers, binary, rendOpt);
    QFile file(path);
    file.open(QIODevice::WriteOnly);
    QTextStream qstream(&file);
    doc.save(qstream, 1);
