...
void GCodeProcessor::TimeProcessor::set_fast_mode()
{
    this->machine_limits.machine_max_acceleration_x.values = {8000.0,8000.0};
    this->machine_limits.machine_max_acceleration_y.values = {8000.0,8000.0};
    this->machine_limits.machine_max_acceleration_z.values = {500.0,500.0};
    this->machine_limits.machine_max_acceleration_e.values = {6000.0,6000.0};
...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    ...
    { EProducer::AnkerPrusa,  " generated by AnkerMake" },
...
void GCodeProcessor::process_file(const std::string& filename, bool apply_postprocess, std::function<void()> cancel_callback)
{
    ...
    if (m_producers_enabled) {
        ...
        try {
            if (m_producer == EProducer::AnkerPrusa || m_producer == EProducer::AnkerMake||m_producer == EProducer::PrusaSlicer || m_producer == EProducer::SuperSlicer || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) {
                //TODO:
                ...
                for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedTimeStatistics::ETimeMode::Count); ++i) {
                    float max_acceleration = get_option_value(m_time_processor.machine_limits.machine_max_acceleration_extruding, i);
                    //m_time_processor.machines[i].max_acceleration = max_acceleration;
                    m_time_processor.machines[i].max_acceleration = 0.0;
                    m_time_processor.machines[i].acceleration = (max_acceleration > 0.0f) ? max_acceleration : DEFAULT_ACCELERATION;
                    m_time_processor.machines[i].max_travel_acceleration = 0.0;//£»get_option_value(m_time_processor.machine_limits.machine_max_acceleration_travel, i);
                    m_time_processor.machines[i].travel_acceleration = (get_option_value(m_time_processor.machine_limits.machine_max_acceleration_travel, i) > 0.0f) ? get_option_value(m_time_processor.machine_limits.machine_max_acceleration_travel, i): DEFAULT_TRAVEL_ACCELERATION;
...
[[nodiscard]] static inline bool parse_number(const std::string_view sv, T &out)
{
...
#else
        if ((!m_producers_enabled || m_producer == EProducer::PrusaSlicer || m_producer == EProducer::Slic3r || m_producer == EProducer::AnkerPrusa) &&
    ...
    bool GCodeProcessor::process_producers_tags(const std::string_view comment)
    {
        ...
        case EProducer::PrusaSlicer:
        case EProducer::SuperSlicer:
        case EProducer::AnkerPrusa:  { return process_prusaslicer_tags(comment); }
        ...
        case EProducer::BambuStudio: { return process_bambu_tags(comment); }
    ...
    bool GCodeProcessor::process_bambu_tags(const std::string_view comment)
    {
        ...
        auto string_to_role = [](const std::string_view role) -> ExtrusionRole
        {
        ...
        if (pos != std::string::npos) {
            const std::string_view role = comment.substr(pos + tag.length());
            m_extrusion_role = string_to_role(role);
            return true;
        }
        ...
        if(pos != std::string::npos){
            fastMode = true;
            this->m_time_processor.set_fast_mode();
        }

        //Galen add 20230720
        tag = "Print Mode:smooth";
        pos = comment.find(tag);
        if(pos != std::string::npos){
            fastMode = true;
            this->m_time_processor.set_fast_mode();
    ...
    void GCodeProcessor::process_G1(const GCodeReader::GCodeLine& line)
    {
        ...
        if(m_on_takepic){
            --m_g1_line_id;
           return;//don't process move line
        }
        ...
        for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedTimeStatistics::ETimeMode::Count); ++i) {
            ...
            do{
                ...
                //It seems that there is no need to calculate the K value in fast mode
                if(fastMode){
                    break;
                }
