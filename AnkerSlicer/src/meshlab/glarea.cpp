...
GLArea::GLArea(QWidget* parent, MultiViewer_Container* mvcont, RichParameterList* current)
    : QGLWidget(parent, mvcont->sharedDataContext()), interrbutshow(false), isImportPedestal(false)
{
    makeCurrent();
    ...
    */
    ...
    //Add by Aden Hu
    setBackgrond(QColor(234, 234, 234), current); 
    sceneBox = Box3m(vcg::Point3f(0.0, 0.0, 0.0), vcg::Point3f(235, 235, 250)); 
}
...
//Add by Aden Hu
void GLArea::setBackgrond(const QColor& color, RichParameterList* current)
{
    if (current == nullptr)
        return;
    ColorValue cv(color);
    RichParameter& rp1 = current->getParameterByName(tr("MeshLab::Appearance::backgroundBotColor"));
    rp1.setValue(cv);

    RichParameter& rp2 = current->getParameterByName(tr("MeshLab::Appearance::backgroundTopColor"));
    rp2.setValue(cv);
    makeCurrent();

    glas.logAreaColor = current->getColor4b(glas.logAreaColorParam());
    glas.backgroundBotColor = current->getColor4b(glas.backgroundBotColorParam());
    glas.backgroundTopColor = current->getColor4b(glas.backgroundTopColorParam());

    glas.baseLightAmbientColor = current->getColor4b(glas.baseLightAmbientColorParam());
    glas.baseLightDiffuseColor = current->getColor4b(glas.baseLightDiffuseColorParam());
    glas.baseLightSpecularColor = current->getColor4b(glas.baseLightSpecularColorParam());

    glas.fancyBLightDiffuseColor = current->getColor4b(glas.fancyBLightDiffuseColorParam());
    glas.fancyFLightDiffuseColor = current->getColor4b(glas.fancyFLightDiffuseColorParam());

    glas.textureMinFilter = current->getEnum(glas.textureMinFilterParam());
    glas.textureMagFilter = current->getEnum(glas.textureMagFilterParam());

    glas.pointDistanceAttenuation = current->getBool(glas.pointDistanceAttenuationParam());
    glas.pointSmooth = current->getBool(glas.pointSmoothParam());
    glas.pointSize = current->getFloat(glas.pointSizeParam());
    glas.wheelDirection = current->getBool(glas.wheelDirectionParam());
    glas.matrixDecimalPrecision = current->getInt(glas.matrixDecimalPrecisionParam());
    glas.currentGlobalParamSet = current;

    update();
}

//Add by Aden Hu
void GLArea::createBBox(const vcg::Point3f& max, const vcg::Point3f& min, float tick, float div)
{
    Box3m box(min, max);
    vcg::Point3f sp = box.Dim();
    Point3m mi = box.min;
    Point3m ma = box.max;
    Point3m d3 = (box.max - box.min) / 2.0;
    Point3m zz(0, 0, 0);
    ...
    GLfloat Xrgb[3] = { 0.0, 0.0, 0.0 };
    GLfloat Yrgb[3] = { 0.0, 0.0, 0.0 };
    GLfloat Zrgb[3] = { 0.0, 0.0, 0.0 };
    glPushMatrix();
...
void GLArea::paintEvent(QPaintEvent* /*event*/)
{
    ...
    if (!this->md()->isBusy())
    {
        ...
        //Add by Aden Hu
        /*MeshModel* mm = md()->getMesh(0);
        if (mm)
        {
            mm->m_bePrintObj = false;
            md()->setVisible(0, false);
            createBBox(sceneBox.max, sceneBox.min, 10.0f, 20.0f);
        } */
    ...
    //Add By Aden Hu
    //if (window && !isImportPedestal)
    //{
    //    isImportPedestal = true;
    //    QString dir = QCoreApplication::applicationDirPath();
    //    dir += tr("/models/10xPrint3DPlaneUpdate.stl");
    //    //window->importMeshWithLayerManagement(dir);
    //    emit openMeshFile(dir);
    //}
...
void GLArea::clickpush()
{
    QPushButton *button = qobject_cast<QPushButton*>( sender());
    qDebug() <<"click,,,,,,,,,,, =  " <<   button->geometry();
    qDebug() <<"map from  =  " <<   button->mapTo(this,QPoint(button->geometry().x(),button->geometry().y()));

    QPoint point = button->mapTo(this,QPoint(button->geometry().x(),button->geometry().y()));
    QWidget *wid = new QWidget(this);
    wid->setGeometry((point.x()+button->width()) + 2,point.y(),200,200);
    QPushButton *po = new QPushButton("click",wid);
    //connect(po,&QPushButton::clicked,this,&GLArea::clickpush);
    po->setGeometry(0,0,100,100);
    wid->show();
    // wid->setStyleSheet("background-color: rgba(255, 255, 255,1);");
    //butt
    //button->ma
    //QWidget *wid = new QWidget(this);

    //wid->setGeometry(button->geometry());
}
...
{
    QFileDialog saveDiag(this, "Save View To File", "./", "View file (*.xml)");
