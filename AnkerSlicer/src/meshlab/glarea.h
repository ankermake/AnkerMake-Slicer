...
public:
    ...
    //Add by Aden Hu
    void setBackgrond(const QColor& color, RichParameterList* current);
    void createBBox(const vcg::Point3f &max, const vcg::Point3f &min, float tick, float div);
    Box3m sceneBox;
...
public:
    ...
    EditTool* getMeshEditor(QAction* action)
    {      
        return actionToMeshEditMap[action];
    }
...
signals:
    ...
    void resizeWidget();
    void openMeshFile(const QString &fileName);
...
private:
    ...
    inline float clipRatioFarDefault() const { return 200.0f;}
