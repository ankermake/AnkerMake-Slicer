...
void MainWindow::updateRecentProjActions()
{
	...
	{
        QString text = QString("&%1 %2").arg(i + 1).arg(QFileInfo(projs[i]).fileName());
...
void MainWindow::updateWindowMenu()
{
	...
	if((mdiarea-> subWindowList().size()>0)){
        ...
        splitModeMenu = windowsMenu->addMenu(QString("&Split current view"));
		...
		// View From SUBmenu
        viewFromMenu = windowsMenu->addMenu(QString("&View from"));
		foreach(QAction *ac, viewFromGroupAct->actions())
			viewFromMenu->addAction(ac);
		
		// Trackball Step SUBmenu
        trackballStepMenu = windowsMenu->addMenu(QString("Trackball step"));
	...
	{
        QString text = QString("&%1. %2").arg(i+1).arg(QFileInfo(w->windowTitle()).fileName());
...
void MainWindow::setSplit(QAction *qa)
{
	...
	if(mvc)
	{
        ...
        if(qa->text() == "&Horizontally")
			mvc->addView(glwClone,Qt::Vertical);
        else if(qa->text() == QString("&Vertically"))
...
void MainWindow::splitFromHandle(QAction *qa )
{
    ...
    if(qa->text() == QString("&Right"))
		point.setX(point.x()+ epsilon);
    else if(qa->text() == "&Left")
		point.setX(point.x()- epsilon);
    else if(qa->text() == QString("&Up"))
		point.setY(point.y()- epsilon);
    else if(qa->text() == "&Down")
		point.setY(point.y()+ epsilon);
	
	int newCurrent = mvc->getViewerByPicking(point);
	mvc->updateCurrent(newCurrent);
	
    if(qa->text() == QString("&Right") || qa->text() == "&Left")
        setSplit(new QAction("&Horizontally", this));
	else
        setSplit( new QAction(QString("&Vertically"), this));
}
...
void MainWindow::startFilter()
{
	...
	if (iFilter != NULL)
	{
		...
		if (!iFilter->isFilterApplicable(action,(*meshDoc()->mm()),missingPreconditions))
		{
            ...
            QMessageBox::warning(this, QString("PreConditions' Failure"), QString("Warning the filter <font color=red>'" + iFilter->filterName(action) + "'</font> has not been applied.<br>"
...
std::pair<QString, QString> MainWindow::extractVertFragFileNames(const QDomElement& root)
{
    ...
    if (root.nodeName() == QString("GLSLang")) {
		QDomElement elem;
...
void MainWindow::saveProject()
{
	...
	if (res == QFileDialog::AcceptSave){
        ...
        path.truncate(path.lastIndexOf("/"));//qDebug()<<"fileName:"<<fileName<<",path:"<<path;
...
//            int findIndex1 = fileName.lastIndexOf("/");
//            QString fileName1=fileName.mid(findIndex1);
//            QString fileNameProject=path+fileName1;

//            qDebug()<<"fileName:"<<fileName<<",path:"<<path<<",fileNameProject:"<<fileNameProject;
...
bool MainWindow::openProject(QString fileName, bool append)
{
	...
	if (fileName.isEmpty())
		fileName =
			QFileDialog::getOpenFileName(
                    this, QString("Open Project File"),
    ...
    //qDebug()<<"id_pm_RunTime:"<<&pm;

	if (ioPlugin == nullptr) {
        QMessageBox::critical(this, QString("Meshlab Opening Error"),
	...
	for (const FileFormat& ff : additionalFilesFormats){
		...
		QString additionalFile =
			QFileDialog::getOpenFileName(
                    this, QString("Open Additional Project File"),
...
//    }
    ...
    if(_currviewcontainer != NULL)
    {
        _currviewcontainer->clearData();
    }
	std::vector<MeshModel*> meshList;
	std::vector<MLRenderingData> rendOptions;
    try
    {
        meshList = meshlab::loadProject(filenames, pm, ioPlugin, *meshDoc(), rendOptions, &meshDoc()->Log, QCallBack);       
	}
	catch (const MLException& e) {
        QMessageBox::critical(this, QString("Meshlab Opening Error"), e.what());
		return false;
    }
...
bool MainWindow::appendProject(QString fileName)
{
	...
	globrendtoolbar->setEnabled(false);
	if (fileName.isEmpty())
        fileNameList = QFileDialog::getOpenFileNames(this, QString("Attach File"), lastUsedDirectory.path(), "All Project Files (*.mlp *.mlb *.aln *.out *.nvm);;MeshLab Project (*.mlp);;MeshLab Binary Project (*.mlb);;Align Project (*.aln);;Bundler Output (*.out);;VisualSFM Output (*.nvm)");
	else
	...
	{
        QMessageBox::critical(this, QString("Meshlab Opening Error"), "Current project is empty, cannot append");
		return false;
	}
...
void MainWindow::newProject(const QString& projName)
{
    ...
    mdiarea->currentSubWindow()->setWindowFlags(Qt::WindowTitleHint);
    ...
    GLArea *gla=new GLArea(/*this*/mdiarea, mvcont, &currentGlobalParams);
    connect(gla, SIGNAL(updateLayerTable()), this, SIGNAL(updateLayerTable()));
    connect(gla,SIGNAL(updateMainWindowMenus()),this,SLOT(updateMenus()));
    connect(this,SIGNAL(dispatchCustomSettings(const RichParameterList&)),gla,SLOT(updateCustomSettingValues(const RichParameterList&)));
...
bool MainWindow::importRaster(const QString& fileImg)
{
	...
	QStringList fileNameList;
	if (fileImg.isEmpty())
        fileNameList = QFileDialog::getOpenFileNames(this,QString("Import Mesh"), lastUsedDirectory.path(), PM.inputImageFormatListDialog().join(";;"));
	else
...
bool MainWindow::importMesh(QString fileName)
{
	...
	for(const QString& fileName : fileNameList) {
		...
		if(!fi.exists()) {
            ...
            QMessageBox::critical(this, QString("Meshlab Opening Error"), errorMsgFormat.arg(fileName));
			return false;
		}
		if(!fi.isReadable()) {
		...
		if(!prePar.isEmpty()) {
			...
			if (showPreOpenParameterDialog){
                RichParameterListDialog preOpenDialog(this, prePar, QString("Pre-Open Options"));
...
bool MainWindow::exportMesh(QString fileName,MeshModel* mod,const bool saveAllPossibleAttributes)
{
	...
	if (!fileName.isEmpty()) {
		...
		try {
            ...
            QMessageBox::critical(this, QString("Meshlab Saving Error"),  e.what());
			saved = false;
		}
		qApp->restoreOverrideCursor();
		updateLayerDialog();
...
void MainWindow::clearToolBarAndMenuBar()
{
    delete  mainToolBar;
    delete  decoratorToolBar;
    delete  editToolBar;
