...
public:
  ...
  static int Save(SaveMeshType &m, QString filename, int mask, CallBackPos *cb=0)
  {
   ...
   */
  static int Save(SaveMeshType &m, QString filename, int mask, bool useMaterialAttribute ,CallBackPos *cb=0)
  {
    ...
    std::string shortFilename = QFile::encodeName(filename).constData();
    //std::string shortFilename(filename.toStdString().c_str());
  ...
  /*
        ...
        */
  inline static int WriteMaterials(std::vector<Material> &materialVec, QString filename, CallBackPos *cb=0)
  {
    std::string fileName = std::string(filename.toStdString());
    fileName+=".mtl";
