...
CopyFileResult copy_file_inner(const std::string& from, const std::string& to, std::string& error_message)
{
	...
	//CXLogDebug("boost::filesystem::permisions before copy error message (this could be irrelevant message based on file system): %s", ec.message().c_str());
	ec.clear();
#ifdef __linux__
	...
	//	CXLogDebug("boost::filesystem::permisions after copy error message (this could be irrelevant message based on file system):  %s", ec.message().c_str());
	return SUCCESS;
}
