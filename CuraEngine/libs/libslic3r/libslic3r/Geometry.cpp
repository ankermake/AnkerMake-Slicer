...
arrange(size_t total_parts, const Vec2d &part_size, coordf_t dist, const BoundingBoxf* bb, Pointfs &positions)
{
    ...
    for (size_t i = 0; i <= cellw-1; ++i) {
        for (size_t j = 0; j <= cellh-1; ++j) {
            coordf_t cx = linint(i + 0.5, 0, cellw, cells_bb.bmin(0), cells_bb.bmax(0));
            coordf_t cy = linint(j + 0.5, 0, cellh, cells_bb.bmin(1), cells_bb.bmax(1));
...
Transformation Transformation::volume_to_bed_transformation(const Transformation& instance_transformation, const BoundingBoxf3& bbox)
{
    ...
    else if (is_rotation_ninety_degrees(instance_transformation.get_rotation()))
    {
        ...
        pts(0, 0) = bbox.bmin.x(); pts(0, 1) = bbox.bmin.y(); pts(0, 2) = bbox.bmin.z();
        pts(1, 0) = bbox.bmin.x(); pts(1, 1) = bbox.bmin.y(); pts(1, 2) = bbox.bmax.z();
        pts(2, 0) = bbox.bmin.x(); pts(2, 1) = bbox.bmax.y(); pts(2, 2) = bbox.bmin.z();
        pts(3, 0) = bbox.bmin.x(); pts(3, 1) = bbox.bmax.y(); pts(3, 2) = bbox.bmax.z();
        pts(4, 0) = bbox.bmax.x(); pts(4, 1) = bbox.bmin.y(); pts(4, 2) = bbox.bmin.z();
        pts(5, 0) = bbox.bmax.x(); pts(5, 1) = bbox.bmin.y(); pts(5, 2) = bbox.bmax.z();
        pts(6, 0) = bbox.bmax.x(); pts(6, 1) = bbox.bmax.y(); pts(6, 2) = bbox.bmin.z();
        pts(7, 0) = bbox.bmax.x(); pts(7, 1) = bbox.bmax.y(); pts(7, 2) = bbox.bmax.z();
