...
{
    this->filename = afilename;
    this->origin   = bbox.bmin - Point(bbox_offset, bbox_offset);
    this->flipY    = aflipY;
    this->f        = boost::nowide::fopen(afilename, "w");
    if (f == NULL)
        return false;
    float w = to_svg_coord(bbox.bmax(0) - bbox.bmin(0) + 2 * bbox_offset);
    float h = to_svg_coord(bbox.bmax(1) - bbox.bmin(1) + 2 * bbox_offset);
    this->height   = h;
    fprintf(this->f,
        "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\n"
        "<!DOCTYPE svg PUBLIC \"-//W3C//DTD SVG 1.0//EN\" \"http://www.w3.org/TR/2001/REC-SVG-20010904/DTD/svg10.dtd\">\n"
...
void SVG::draw(const Clipper3r::Path &polygon, double scale, std::string stroke, coordf_t stroke_width)
{
    this->stroke = stroke;
    this->path(this->get_path_d(polygon, scale, true), false, stroke_width, 1.f);
}

void SVG::draw(const Clipper3r::Paths &polygons, double scale, std::string stroke, coordf_t stroke_width)
{
    for (Clipper3r::Paths::const_iterator it = polygons.begin(); it != polygons.end(); ++ it)
...
std::string SVG::get_path_d(const Clipper3r::Path &path, double scale, bool closed) const
{
    std::ostringstream d;
    d << "M ";
    for (Clipper3r::Path::const_iterator p = path.begin(); p != path.end(); ++p) {
