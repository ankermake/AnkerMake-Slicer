...
void stl_generate_shared_vertices(stl_file *stl, indexed_triangle_set &its, ccglobal::Tracer* tracer)
{
	...
	for (uint32_t facet_idx = 0; facet_idx < stl->stats.number_of_facets; ++ facet_idx) {
		SPLIT_PROGRESS_MSG_BREAK(facet_idx, 100, stl->stats.number_of_facets, tracer, "");
...
bool its_write_off(const indexed_triangle_set &its, const char *file)
{
	...
	if (fp == nullptr) {
		...
		//CXLogError("stl_write_ascii: Couldn't open %s for writing",file);
		return false;
	}
...
bool its_write_vrml(const indexed_triangle_set &its, const char *file)
{
	...
	if (fp == nullptr) {
		...
		//CXLogError("stl_write_vrml: Couldn't open %s  for writing", file);
		return false;
	}
...
bool its_write_obj(const indexed_triangle_set &its, const char *file)
{
  	...
  	if (fp == nullptr) {
		...
		//CXLogError("stl_write_obj: Couldn't open %s for writing",file);
    	return false;
  	}
            ...
            if (nbr_face != -1) {
				...
				if (vertices != nullptr) {
	            	...
	            	if (nbr_vnot < 3) {
						...
						int a = its.indices[nbr_face][(nbr_vnot + 1) % 3];
						int b = vertices[(nbr_idx + 1) % 3];
						int c = its.indices[nbr_face][(nbr_vnot + 2) % 3];
						int d = vertices[nbr_idx];
