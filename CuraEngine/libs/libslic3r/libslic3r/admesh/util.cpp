...
void stl_verify_neighbors(stl_file *stl, ccglobal::Tracer* tracer)
{
	...
	for (uint32_t i = 0; i < stl->stats.number_of_facets; ++ i) {
		...
		for (int j = 0; j < 3; ++ j) {
			...
			if (edge_a.p1 != edge_b.p1 || edge_a.p2 != edge_b.p2) {
				...
				//CXLogInfo("edge %d  of facet %d doesn't match edge %d of facet", j,i,(vnot+1),neighbor);
...
void stl_get_size(stl_file *stl, ccglobal::Tracer* tracer)
{
...
void stl_calculate_volume(stl_file *stl, ccglobal::Tracer* tracer)
{
