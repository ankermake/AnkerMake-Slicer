...
bool stl_fix_normal_directions(stl_file *stl, ccglobal::Tracer* tracer)
{
    bool forceExiten = false;
    ...
    // of it being wrong randomly are low if most of the triangles are right:
  	if (check_normal_vector(stl, facet_num, 0)) {
    	reverse_facet(stl, facet_num);
...
void stl_fix_normal_values(stl_file *stl, ccglobal::Tracer* tracer)
{
    for (uint32_t i = 0; i < stl->stats.number_of_facets; ++i) {
...
void stl_reverse_all_facets(stl_file *stl, ccglobal::Tracer* tracer)
{
	stl_normal normal;
  	for (uint32_t i = 0; i < stl->stats.number_of_facets; ++ i) {
    	reverse_facet(stl, i);
