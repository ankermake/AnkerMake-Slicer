		...
		auto change_vertices = [stl](int facet_num, int vnot, stl_vertex new_vertex)
		{
			...
			for (;;) {
				...
				if (facet_num == first_facet) {
					...
					//CXLogInfo("Back to the first facet changing vertices: probably a mobius part. Try using a smaller tolerance or don't do a nearby check.");
					return;
				}
			}
		};
...
void stl_check_facets_exact(stl_file *stl, ccglobal::Tracer* tracer)
{
		...
		stl_facet &facet = stl->facet_start[i];
	  	if (facet.vertex[0] == facet.vertex[1] || facet.vertex[1] == facet.vertex[2] || facet.vertex[0] == facet.vertex[2]) {
			...
			stl->neighbors_start.pop_back();
		  	...
		  	++ i;
		SPLIT_PROGRESS_MSG_BREAK(i, 100, stl->stats.number_of_facets, tracer, "");
  	}
	RESET_PROGRESS_PERCENT(tracer, 0.6);

  	// Initialize hash table.
  	HashTableEdges hash_table(stl->stats.number_of_facets);
	for(int i = 0; i < stl->neighbors_start.size(); i++)
	{
		stl_neighbors& neighbor = stl->neighbors_start[i];
		neighbor.reset();
		SPLIT_PROGRESS_MSG_BREAK(i, 100, stl->neighbors_start.size(), tracer, "");
	}
...
void stl_check_facets_nearby(stl_file *stl, float tolerance, ccglobal::Tracer* tracer)
{
...
void stl_remove_unconnected_facets(stl_file *stl, ccglobal::Tracer* tracer)
{
		    	...
		    	if (neighbors.neighbor[i] != -1) {
			    	int &other_face_idx = stl->neighbors_start[neighbors.neighbor[i]].neighbor[(neighbors.which_vertex_not[i] + 1) % 3];
			  		if (other_face_idx != stl->stats.number_of_facets) {
						...
						//CXLogInfo("in remove_facet: neighbor = %d  numfacets = %d this is wrong", other_face_idx, stl->stats.number_of_facets);
						return;
			  		}
	...
	RESET_PROGRESS_PERCENT(tracer, 0.5);
		...
		for (int j = 0; j < 3; ++ j) {
	  		...
	  		for (;;) {
	    		...
	    		if (facet_num == first_facet) {
					...
					//CXLogInfo("Back to the first facet filling holes: probably a mobius part. Try using a smaller tolerance or don't do a nearby check.");
	      			return;
	    		}
	  		}
		}
