...
static FILE* stl_open_count_facets(stl_file *stl, const char *file) 
{
  	...
  	if (fp == nullptr) {
		...
		//CXLogError("stl_open_count_facets: Couldn't open %s for reading",file);
    	return nullptr;
  	}
  	// Find size of file.
  	fseek(fp, 0, SEEK_END);
	...
	unsigned char chtest[128];
  	if (! fread(chtest, sizeof(chtest), 1, fp)) {
		...
		//CXLogError("stl_open_count_facets: The input is an empty file: %s" ,file);
    	fclose(fp);
    	return nullptr;
  	}
  	...
  	if (stl->stats.type == binary) {
    	...
    	if (((file_size - HEADER_SIZE) % SIZEOF_STL_FACET != 0) || (file_size < STL_MIN_FILE_SIZE)) {
			...
			//CXLogError("stl_open_count_facets: The file %s has the wrong size.",file);
      		fclose(fp);
      		return nullptr;
    	}
		...
		//if (!header_num_faces_read || num_facets != header_num_facets)
			...
			//CXLogInfo("stl_open_count_facets: Warning: File size doesn't match number of facets in the header: ",file);
  	}
  	...
  	for (uint32_t i = first_facet; i < stl->stats.number_of_facets; ++ i) {
    	...
    	if (stl->stats.type == binary) {
			...
			if (res_normal != 3 || res_outer_loop != 0 || res_vertex1 != 3 || res_vertex2 != 3 || res_vertex3 != 3 || ! endloop_ok || ! endfacet_ok) {
				...
				//CXLogError("Something is syntactically very wrong with this ASCII STL! ");
				return false;
			}
