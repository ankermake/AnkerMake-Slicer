...
bool stl_print_neighbors(stl_file *stl, char *file)
{
	...
	if (fp == nullptr) {
		...
		//CXLogError("stl_print_neighbors: Couldn't open %s for writing",file);
    	return false;
  	}
...
bool stl_write_binary(stl_file *stl, const char *file, const char *label)
{
	...
	if (fp == nullptr) {
		...
		//CXLogError("stl_write_binary: Couldn't open %s for writing",file);
    	return false;
  	}
...
bool stl_write_quad_object(stl_file *stl, char *file)
{
	...
	if (fp == nullptr) {
		...
		//CXLogError("stl_write_quad_object: Couldn't open %s for writing",file);
		return false;
	}
