...
public:
    void init(const TriangleMesh& tm, ccglobal::Tracer* tracer = nullptr)
    {
        m_tree = AABBTreeIndirect::build_aabb_tree_over_indexed_triangle_set(
            tm.its.vertices, tm.its.indices, 0, tracer);
    }
...
IndexedMesh::IndexedMesh(const TriangleMesh& tmesh, ccglobal::Tracer* tracer)
    : m_aabb(new AABBImpl()), m_tm(&tmesh)
{
    auto&& bb = tmesh.bounding_box();
    m_ground_level += bb.bmin(Z);

    // Build the AABB accelaration tree
    m_aabb->init(tmesh, tracer);
}

IndexedMesh::IndexedMesh(const TriangleMesh& tmesh, const IndexedMesh& other)
    : m_tm(&tmesh)
    , m_aabb(new AABBImpl(*other.m_aabb))
{
    m_ground_level = other.m_ground_level;
    m_gnd_offset = other.m_gnd_offset;
}
