...
Contour3D pinhead(double r_pin, double r_back, double length, size_t steps)
{
    ...
    auto &&s3 = sphere(r_pin+0.2, make_portion(0 , PI), detail);

    for (auto &p : s2.points) p.z() += h;

    for (auto &p : s3.points) p.z() += h;
...
Contour3D ball(double radius,  size_t steps)
{
    assert(steps > 0);
    assert(radius > 0.);
    ...
    // rules of tangent circles from which we can derive (using simple
    // triangles the following relations:
    auto&& s = sphere(radius, make_portion(0, PI), detail);
...
Contour3D pinHeadCylinder(double r_pin, double r_back, double length, size_t steps)
{
    assert(steps > 0);
    assert(length >= 0.);
    assert(r_back > 0.);
    ...
    Contour3D ret;
    const Vec3d sp(0.0, 0.0, 0.0);
    auto& points = ret.points;
    auto& indices = ret.faces3;
    points.reserve(2 * steps);
    double detail = 2 * PI / steps;
    const double h = r_back + r_pin + length;
    ...
    for (int i = 0; i < steps; ++i) {
        double phi = i * detail;
        double ex = endp(X) + r_pin * std::cos(phi);
        double ey = endp(Y) + r_pin * std::sin(phi);
        points.emplace_back(ex, ey, endp(Z));
    }
    ...
    for (int i = 0; i < steps; ++i) {
        double phi = i * detail;
        double x = jp(X) + r_back * std::cos(phi);
        double y = jp(Y) + r_back * std::sin(phi);
        points.emplace_back(x, y, jp(Z));
    }
