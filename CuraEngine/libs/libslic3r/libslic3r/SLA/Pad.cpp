...
struct PadConfig3D {
    double thickness, height, wing_height, slope, brim_size_off;
    ...
    explicit PadConfig3D(const PadConfig &cfg2d)
        ...
        , brim_size_off{cfg2d.brim_size_mm }
    {}
    ...
    inline double bottom_offset_brim() const
    {
        return brim_size_off;
    }
};
...
{
    Clipper3r::PolyTree ptree = union_pt(bp);

    PadSkeleton ret;
    ret.inner.reserve(size_t(ptree.Total()));
    ret.outer.reserve(size_t(ptree.Total()));

    for (Clipper3r::PolyTree::PolyNode *node : ptree.Childs) {
        ExPolygon poly(ClipperPath_to_Slic3rPolygon(node->Contour));
        for (Clipper3r::PolyTree::PolyNode *child : node->Childs) {
...
bool add_cavity(Contour3D &pad, ExPolygon &top_poly, const PadConfig3D &cfg,
                ThrowOnCancel thr)
{
    auto logerr = []{/*CXLogError("Could not create pad cavity");*/};
    ...
    Contour3D cavityWall = walls(inner_base.contour, middle_base.contour, z_min, z_max);
	#if 1
    Vec3d normalUp(0.0, 0.0, -1.0);
    for (Vec3i& faceindex : cavityWall.faces3)
    {
        Vec3d pt0 = cavityWall.points[faceindex[0]];
        Vec3d pt1 = cavityWall.points[faceindex[1]];
        Vec3d pt2 = cavityWall.points[faceindex[2]];
        Vec3d pt01 = pt1 - pt0;
        Vec3d pt02 = pt2 - pt0;
        Vec3d normal = pt01.cross(pt02);
        if (normal.dot(normalUp) < 0)
        {
            std::swap(faceindex[1], faceindex[2]);
        }
    }
	#endif
    pad.merge(cavityWall);
    thr();
                                    ...
                                    ThrowOnCancel      thr)
{
    ...
    ExPolygons output;
    for (auto& o : skeleton) {
        //auto&& smp = o.simplify(scaled<double>(cfg.wing_height/ std::tan(cfg.slope)));
        auto&& smp = o.simplify(scaled<double>(0.1));
        output.insert(output.end(), smp.begin(), smp.end());
    }

    for (const ExPolygon &pad_part : output) {
        ExPolygon top_poly{pad_part};
        //ExPolygon bottom_poly =offset_contour_only(pad_part, -scaled(cfg.bottom_offset_brim()));
        ExPolygon bottom_poly{ pad_part };

        top_poly =offset_contour_only(top_poly, scaled(cfg.bottom_offset()));

        if (bottom_poly.empty()) continue;
        thr();
        
        double z_min = -cfg.height, z_max = 0;
        Contour3D contourWall = walls(top_poly.contour, bottom_poly.contour, z_max, z_min);
		#if 1
        Vec3d normalDown(0.0,0.0,-1.0);
        for (Vec3i& faceindex : contourWall.faces3)
        {
            Vec3d pt0 = contourWall.points[faceindex[0]];
            Vec3d pt1 = contourWall.points[faceindex[1]];
            Vec3d pt2 = contourWall.points[faceindex[2]];
            Vec3d pt01 = pt1 - pt0;
            Vec3d pt02 = pt2 - pt0;
            Vec3d normal = pt01.cross(pt02);
            if (normal.dot(normalDown) < 0)
            {
                std::swap(faceindex[1], faceindex[2]);
            }
        }
		#endif
        ret.merge(contourWall);
                   ...
                   ThrowOnCancel             thrfn)
{
    ...
    for(ExPolygons& o : out)
        for(ExPolygon& e : o) {
            auto&& exss = e.simplify(scaled<double>(0.05));
    ...
    for(auto& o : utmp) {
        auto&& smp = o.simplify(scaled<double>(0.05));
        output.insert(output.end(), smp.begin(), smp.end());
    }
}
...
void pad_blueprint(const TriangleMesh &mesh,
                   ...
                   ThrowOnCancel       thrfn)
{
    float gnd = float(mesh.bounding_box().bmin(Z));
