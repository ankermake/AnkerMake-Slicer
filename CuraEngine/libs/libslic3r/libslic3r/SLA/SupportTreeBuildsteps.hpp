...
// The minimum distance for two support points to remain valid.
    const double /*constexpr*/ D_SP = 1.0;//0.1;
...
public:
    PillarIndex& operator=( PillarIndex&);
    ...
    template<class...Args> inline void guarded_remove(Args&&...args)
    {
        std::lock_guard<Mutex> lck(m_mutex);
        m_index.remove(std::forward<Args>(args)...);
    }
...
public:
    ...
    long m_addHeadsize = 0;
    PtIndices m_ipillar_onmodel;
private:
    std::vector<SupportPoint> m_support_pts;
    ...
    std::pair<PointSet, std::vector<SupportTreeNode2MeshLink>>               m_pillarPoints;
    std::vector<SupportTreeNode2MeshLink> m_TreeNodesNext;
...
public:
    // A spatial index to easily find strong pillars to connect to,Save the pillar endpoint in the spatial index
    PillarIndex m_pillar_index;
    PillarIndex m_pillar_index_start;
...
public:
    ...
    IndexedMesh::hit_result cone_mesh_intersect(
        const Vec3d& s,
        const Vec3d& dir,
        double       r_pin,
        double       r_back,
        double       width,
        double       sd=0.0);
    IndexedMesh::hit_result square_mesh_intersect(
        const Vec3d& s,
        const Vec3d& dir,
        double       r_pin,
        double       r_back,
    ...
    void setThrowOnCancel(ThrowOnCancel fun)
    {
        m_thr = fun;
    }
...
public:
    ...
    bool interconnect( Pillar& pillar,  Pillar& nextpillar, std::vector<SupportTreeNode2MeshLink> &outintermeshlink);
    ...
    bool create_ground_pillar(const Vec3d &jp,
                              ...
                              double       radius,
                              long         head_id /*= SupportTreeNode::ID_UNSET*/, std::vector<SupportTreeNode2MeshLink>& outTreeNodelink);

    bool create_direct_ground_pillar(const Vec3d &jp,
                              const Vec3d &sourcedir,
                              double       radius,
                              std::vector<SupportTreeNode2MeshLink>& outTreeNodelink);
    void add_pillar_base(long pid)
    {
        m_builder.add_pillar_base(pid, m_cfg.base_height_mm, m_cfg.base_top_radius_mm, m_cfg.base_radius_mm);
    }
    ...
    std::optional<DiffBridge> search_widening_path(const Vec3d &jp,
                                                   ...
                                                   double       radius,
                                                   double       new_radius);
    std::optional<DiffBridge> search_widening_path_pillar(const Vec3d &jp,
                                                   const Vec3d &dir,
                                                   double       radius,
                                                   double       new_radius);

public:
    SupportTreeBuildsteps(SupportTreeBuilder & builder, const Slic3r::sla::IndexedMesh& emesh,
        const SupportPoints& supportPoints, const SupportTreeConfig& config);
    SupportTreeBuildsteps(SupportTreeBuilder& builder, const Slic3r::sla::IndexedMesh& emesh);

    void setData(const SupportPoints& supportPoints, const SupportTreeConfig& config);
    ...
    inline void merge_result() { 
        std::cout  << __LINE__ << "  " << __FUNCTION__ << "  " << std::endl;
        m_builder.merged_mesh(); }

    static bool create(SupportTreeBuilder& builder, const TriangleMesh& mesh, const IndexedMesh& indexedMesh,
        const SupportPoints& supportPoints, const SupportTreeConfig& config);

    static bool execute(SupportTreeBuilder & builder, const TriangleMesh& mesh, const IndexedMesh& indexedMesh, 
        const SupportPoints& supportPoints, const SupportTreeConfig& config);

    bool add_pinheadSupport(const Head& source, int id);
    void dealOneHeadPre(std::vector<Head> &heads);
    void releaseSupportdata();
    long search_pillar(PointIndex &spindex,Vec3d pt);
    std::vector<PointIndexEl> searchSidePillarPoint(PointIndex& spindex, const PointIndexEl& e1);
    long searchSidePillarPoint(PointIndex& spindex, Vec3d pt, PointIndexEl& outptIndexE1);
    bool bridge_to_nearpillar(const Vec3d& pt, double radius, long  nearpillar_id, std::vector<SupportTreeNode2MeshLink>& outbrilinks);
    void updateSupportParamsCfg(const SupportTreeConfig &cfg);
    bool connect_to_model_body(const Vec3d& pos, double radius, std::vector<SupportTreeNode2MeshLink>& outTreeNodelink);
    void CX_classify();
    void CX_routing_to_ground();
    void CX_routing_to_model();
    bool createSmallPillar(SupportTreeNode2MeshLink ptTreeNode,std::vector<SupportTreeNode2MeshLink> &outnewmeshlk);
    void filterDown();
    void CX_classify_simple();
};
