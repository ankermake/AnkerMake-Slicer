...
void TriangleMesh::repair(bool update_shared_vertices, ccglobal::Tracer* tracer)
{
   ...
   // //BOOST_LOG_TRIVIAL(debug) << "TriangleMesh::repair() started";
...
#endif /* SLIC3R_TRACE_REPAIR */
        stl_remove_unconnected_facets(&stl, tracer);
	    assert(stl_validate(&this->stl));
    }
...
#endif /* SLIC3R_TRACE_REPAIR */
    bool isForceExit = stl_fix_normal_directions(&stl, tracer);
    assert(stl_validate(&this->stl));
...
#endif /* SLIC3R_TRACE_REPAIR */
    stl_fix_normal_values(&stl, tracer);
    assert(stl_validate(&this->stl));
    ...
    if (!isForceExit)
    {
        stl_calculate_volume(&stl,tracer);
        assert(stl_validate(&this->stl));
    }
...
BoundingBoxf3 TriangleMesh::bounding_box() const
{
    BoundingBoxf3 bb;
    bb.defined = true;
    bb.bmin = this->stl.stats.min.cast<double>();
    bb.bmax = this->stl.stats.max.cast<double>();
    return bb;
}
...
void TriangleMesh::require_shared_vertices(ccglobal::Tracer* tracer)
{
    ...
    if (this->its.vertices.empty()) {
        ...
        stl_generate_shared_vertices(&this->stl, this->its, tracer);
    }
...
void TriangleMesh::clone(const TriangleMesh& tm)
{
    stl.clone(tm.stl);
    its.clone(tm.its);
    repaired = tm.repaired;
}
    ...
    std::vector<Polygons>* layers, throw_on_cancel_callback_type throw_on_cancel) const
{
    ...
    //CXLogDebug("TriangleMeshSlicer::slice");
    ...
    */
    ...
    //CXLogError("TriangleMeshSlicer::_slice_do");
    std::vector<IntersectionLines> lines(z.size());
    {
        boost::mutex lines_mutex;
#ifdef SLICE3R_USE_TBB
        tbb::parallel_for(
        ...
        range.reserve(this->mesh->stl.stats.number_of_facets);
        for (int i = 0; i < this->mesh->stl.stats.number_of_facets; ++i) range.emplace_back(i);
        std::for_each(std::execution::par, 
            range.begin(), range.end(), [&lines, &lines_mutex, &z, throw_on_cancel, this](auto& facet_idx)
            {
               if ((facet_idx & 0x0ffff) == 0)
                   throw_on_cancel();
                this->_slice_do(facet_idx, &lines, &lines_mutex, z);
    ...
    //CXLogDebug("TriangleMeshSlicer::_make_loops_do");
    ...
    {
        std::vector<int> range;
        range.reserve(z.size());
        for (int i = 0; i < z.size(); ++i) range.emplace_back(i);
        std::for_each(std::execution::par,
            range.begin(), range.end(),
            [&lines, &layers, mode, alternate_mode_first_n_layers, alternate_mode, throw_on_cancel, this](auto& line_idx)
            {
                if ((line_idx & 0x0ffff) == 0)
                    throw_on_cancel();
    ...
    //CXLogDebug("TriangleMeshSlicer::slice finished");
...
{
    //CXLogError(" Enter TriangleMeshSlicer::_slice_do ....");
    ...
    #endif /* SLIC3R_TRIANGLEMESH_DEBUG */
    //CXLogError(" before for (std::vector<float>::const_iterator it = min_layer; it != max_layer; ++ it) ");
    ...
    //CXLogError(" End TriangleMeshSlicer::_slice_do ....");
}
    ...
    std::vector<ExPolygons>* layers, throw_on_cancel_callback_type throw_on_cancel) const
{
    ...
    //CXLogDebug("TriangleMeshSlicer::make_expolygons in parallel - start");
 
#if 0
    //test ploy save
    for (int index = 0; index < layers_p.size(); index++)
    {
        char ch[12];
        sprintf(ch, "%d", index);
        std::string file;
        ...
        if (out.is_open())
        {
            Polygons& ploys = layers_p[index];
            int size = (int)ploys.size();
            out.write((const char*)&size, sizeof(int));
            for (int j = 0; j < ploys.size(); j++)
            {
                size = (int)ploys[j].size();
                out.write((const char*)&size, sizeof(int));
                for (size_t jj = 0; jj < size; jj++)
                {
                    out.write((const char*)(&ploys[j].points[jj]), sizeof(Point));
                }
            }
        }
    }
...
#else
    {
        ...
        std::for_each(std::execution::par,
            range.begin(), range.end(),
            [&layers_p, mode, alternate_mode_first_n_layers, alternate_mode, closing_radius, layers, throw_on_cancel, this](auto& layer_id) {
#ifdef SLIC3R_TRIANGLEMESH_DEBUG
				printf("Layer %zu (slice_z = %.2f):\n", layer_id, z[layer_id]);
#endif
				throw_on_cancel();
    ...
    //CXLogDebug("TriangleMeshSlicer::make_expolygons in parallel - end");
}
...
void TriangleMeshSlicer::cut(float z, TriangleMesh* upper, TriangleMesh* lower) const
{
    ...
    //CXLogDebug("TriangleMeshSlicer::cut - slicing object");
    ...
    if (upper != nullptr) {
        ...
        //CXLogDebug("TriangleMeshSlicer::cut - triangulating upper part");
        ExPolygons section;
    ...
    if (lower != nullptr) {
        ...
        //CXLogDebug("TriangleMeshSlicer::cut - triangulating lower part");
        ExPolygons section;
    ...
    //CXLogDebug("TriangleMeshSlicer::cut - updating object sizes");
