...
{ 
    ...
    if (! points.empty()) {
        ...
        bbox.bmin(0) = bbox.bmax(0) = (coord_t)round(c * cur_x - s * cur_y);
        bbox.bmin(1) = bbox.bmax(1) = (coord_t)round(c * cur_y + s * cur_x);
        for (++it; it != points.end(); ++it) {
            double cur_x = (double)(*it)(0);
            double cur_y = (double)(*it)(1);
            coord_t x = (coord_t)round(c * cur_x - s * cur_y);
            coord_t y = (coord_t)round(c * cur_y + s * cur_x);
            bbox.bmin(0) = std::min(x, bbox.bmin(0));
            bbox.bmin(1) = std::min(y, bbox.bmin(1));
            bbox.bmax(0) = std::max(x, bbox.bmax(0));
            bbox.bmax(1) = std::max(y, bbox.bmax(1));
        }
        bbox.defined = true;
    }
    return bbox;
